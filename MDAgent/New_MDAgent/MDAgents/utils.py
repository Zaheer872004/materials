...
def setup_model(model_name):
    if 'gemini' in model_name:
        ...
        return genai, None
    elif '"llama-3.2-90b-vision-preview' in model_name:
        groq.configure(api_key=os.environ['groq_api_key'])
        return groq, None
...
def process_basic_query(question, examplers, model, args):
    medical_agent = Agent(
        instruction='You are a helpful medical agent.', role='medical expert', 
        model_info=model
        )
    new_examplers = []
    ...
    single_agent = Agent(
        instruction='You are a helpful assistant that answers multiple choice questions about medical knowledge.', role='medical expert', 
