...
# from groqapi import GroqAPI  # Importing GroqAPI for integration
from pptree import *

class Agent:
    def __init__(self, instruction, role, examplers=None, model_info='llama-3.3-70b-versatile', img_path=None):
        """
        Initialize the agent with a specific role, instruction, and model configuration.
        Supports GROQ API.
        """
        ...
        if self.model_info == 'groq':
            ...
            if examplers is not None:
                # Add example questions and answers to the conversation history
    ...
    def temp_responses(self, message, img_path=None):
        """
        Generate responses with varying parameters (for GROQ API).
        """
        if self.model_info == 'groq':      
            self.messages.append({"role": "user", "content": message})
            
            temperatures = [0.0]  # Currently using a single temperature value
            responses = {}
            for temperature in temperatures:
                ...
                responses[temperature] = response['content']
            return responses

class Group:
    def __init__(self, goal, members, question, examplers=None):
        """
        Create a collaborative group of agents to solve a problem collectively.
        """
        self.goal = goal
        self.members = []
        for member_info in members:
            # Initialize each agent with a specific role and expertise
            _agent = Agent('You are a {} who {}.'.format(member_info['role'], member_info['expertise_description'].lower()), role=member_info['role'], model_info='groq')
    ...
    def interact(self, comm_type, message=None, img_path=None):
        """
        Facilitate communication and collaboration among group members.
        """
        if comm_type == 'internal':
            ...
            # Generate a delivery prompt for the lead member
            ...
            # Gather investigation summaries from assistant members
            investigations = []
            for a_mem in assist_members:
            ...
            # Combine investigations into a single prompt
            gathered_investigation = ""
            for investigation in investigations:
                gathered_investigation += "[{}]\n{}\n".format(investigation[0], investigation[1])

            # If example cases are provided, include them in the final prompt
...
def parse_hierarchy(info, emojis):
    """
    Parse hierarchy information and represent it as a tree structure.
    """
...
def parse_group_info(group_info):
    """
    Parse information about group structure and return it as a dictionary.
    """
...
def setup_model(model_name):
    """
    Configure and return the appropriate model client based on the model name.
    """
...
def load_data(dataset):
    """
    Load test and example data from dataset files.
    """
    test_qa = []
    examplers = []
...
def create_question(sample, dataset):
    """
    Create a formatted question from a sample.
    """
    if dataset == 'medqa':
...
#     medical_agent = Agent(instruction='You are a medical expert who conducts initial assessment and your job is to decide the difficulty/complexity of the medical query.', role='medical expert', model_info='groq')
...
def process_advanced_query(question, model, args):
    ...
    return final_decision    # ...existing code...
    
    # Remove the duplicate determine_difficulty function
    def determine_difficulty(question, difficulty):
        if difficulty != 'adaptive':
            return difficulty
    ...
    # Update the Agent class to handle other model_info values
    class Agent:
        def __init__(self, instruction, role, examplers=None, model_info='llama-3.3-70b-versatile', img_path=None):
            """
            Initialize the agent with a specific role, instruction, and model configuration.
            ...
            if self.model_info == 'groq':
                ...
                # Initialize GPT-3.5 client (example placeholder)
                self.client = GPT35Client(api_key=os.environ['gpt35_api_key'])
            else:
        ...
        def temp_responses(self, message, img_path=None):
            """
            Generate responses with varying parameters.
            """
            self.messages.append({"role": "user", "content": message})
            temperatures = [0.0]
            responses = {}
            for temperature in temperatures:
                if self.model_info == 'groq':
                    response = self.client.chat(messages=self.messages, temperature=temperature)
                elif self.model_info == 'gpt-3.5':
                    response = self.client.chat(messages=self.messages, temperature=temperature)
                else:
                    raise ValueError(f"Unsupported model: {self.model_info}")
